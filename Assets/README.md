...
`Sources/Furball2D/Assets/Clips/left-profile/` 包含应用实际播放的 8 段 HEVC with Alpha 视频：
...
这些导出文件可以由 `Scripts/build-assets.sh` 完整重建。切点、循环策略和画布参数记录在脚本与 `Sources/Furball2D/Assets/manifest.json` 中。
