...
The current shipping app selects bundled packs but keeps the runtime library read-only: import, export, replacement, deletion, creation requests, and local model launching are disabled. New packs are produced and validated through the developer-side repository workflow, then bundled at build time. Any future provider must preserve this contract and must not bypass the runtime file-safety boundary.
