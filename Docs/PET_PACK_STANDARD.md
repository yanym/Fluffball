...
- Furball's production atlas is transparent lossless WebP at 3072×4576, 8 columns × 11 rows, 384×416 cells, `spriteVersionNumber: 2`, and `assetScale: 2`. Generate every action row natively for the target clarity, process the highest-resolution approved source directly into the final cells without a downsample/upscale intermediate, keep registration enlargement at or below 1.25×, and include `QA/clarity.json`. The old 1536×2288 form is rejected. Review the exact first `stand.idle` cell during a real appearance switch; if it is soft, rebuild the complete idle row from the native identity master.
...
- Image mode now provides one-style stand, sit, genuinely horizontal closed-eye sleep, wake, phased locomotion, cute actions, and 16-direction gaze. A standing low-head pose is drowsy/sniffing rather than sleep. Right-facing art prefers `rightAnimation` and mirrors only as a fallback. Gait starts contain at least four ordered poses; loops continue at the next contact pose and publish all eight distinct poses. Walk, jog, and run keep one pose order so runtime tier changes preserve gait phase without leg snaps.
