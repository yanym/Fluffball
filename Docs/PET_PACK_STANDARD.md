...
- Furball 生产图集使用 v2 语义布局的 2× 扩展：透明无损 WebP、3072×4576、8 列×11 行、每格 384×416、`spriteVersionNumber: 2`、`assetScale: 2`。1536×2288 / 192×208 仅用于兼容旧包，不再作为新素材默认值。每个动作行必须原生生成到目标清晰度，配准放大不得超过 1.25×，并提交 `QA/clarity.json`；禁止把旧 1× 格子放大后冒充高清。`animations` 声明行、有效帧数、逐帧时长、循环和短溶解比例；`bindings` 将任意语义 ID 映射到动画。
...
- 视频规格：1280×720、120 fps、HEVC with Alpha、`hvc1`、无音轨；24 fps 原片先在完整绿幕轮廓上做双向运动补偿插帧，再抠像和归一化，不能只复制帧。`clips` 为每个语义 ID 声明相对路径与循环属性。
...
- Furball's production atlas is the 2× extension of the v2 semantic layout: transparent lossless WebP at 3072×4576, 8 columns × 11 rows, 384×416 cells, `spriteVersionNumber: 2`, and `assetScale: 2`. Generate every action row natively for the target clarity, keep registration enlargement at or below 1.25×, and include `QA/clarity.json`; enlarging legacy 1× cells does not qualify as HD. The 1536×2288 / 192×208 form remains a legacy import path, not the default for new assets.
