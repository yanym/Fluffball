...
- Furball's production atlas is transparent lossless WebP at 3072×4576, 8 columns × 11 rows, 384×416 cells, `spriteVersionNumber: 2`, and `assetScale: 2`. Generate every action row natively for the target clarity, process the highest-resolution approved source directly into the final cells without a downsample/upscale intermediate, keep registration enlargement at or below 1.25×, and include `QA/clarity.json`. The old 1536×2288 form is rejected.
