...
- Settings uses a sidebar with General, Appearance, Behavior, Desktop Interaction, Pet Group, Dialogue, and a read-only Pet Library. The menu-bar item keeps only immediate actions, and video blending is absent in image mode.
...
- Pet Library selects the active bundled profile and shows personality and memory state. Runtime import, export, deletion, and model-launching paths are disabled by the product safety boundary.
...
- The shipping app is read-only with respect to user files and folders. It contains no Finder AppleScript and no runtime create/copy/move/trash/delete path; `Scripts/audit-runtime-safety.sh` is a mandatory packaging gate.
...
The Pet Pack creation Skill remains in the source tree at [`Sources/Furball2D/CreatorSkill`](Sources/Furball2D/CreatorSkill) for developer-side asset production. It is not launched by the shipping app and cannot write into the user’s library.
...
- “Settings”: the sidebar directly manages General, Appearance, Behavior, Desktop Interaction, Pet Group, Dialogue, and the read-only Pet Library. Pet size and window level live here instead of in the menu-bar item; video blending appears only for Live Motion.
- “Pet Library”: select a bundled pet and inspect its profile. It cannot import, export, replace, or remove files at runtime.
...
- 宠物大小支持 60%–140% 无级调节。
- “设置”窗口使用左侧栏直接提供通用、外观、行为、桌面互动、宠物群、对话和只读宠物素材库；顶栏菜单只保留即时动作入口，视频专用的柔和过渡不会出现在图片模式中。
- “宠物群”可让用户勾选任意支持图片模式的宠物一起出现。每只宠物独立进行 60 Hz 平滑移动，会在抵达目标后及时停下、在屏幕边缘反向，并定期会合做出邀请玩耍、歪头、挥爪和开心舞等互动。
- “宠物素材库”只负责切换内置宠物并展示性格和记忆。正式 App 已禁用运行时导入、导出、删除与模型启动路径。
...
- 正式 App 对用户文件和文件夹保持只读：不包含 Finder AppleScript，也没有运行时创建、复制、移动、废纸篓或删除路径；打包前必须通过 `Scripts/audit-runtime-safety.sh`。
...
Pet Pack 创建 Skill 保存在 [`Sources/Furball2D/CreatorSkill`](Sources/Furball2D/CreatorSkill)，仅供开发阶段离线制作素材。正式 App 不会启动该 Skill，也不会向用户素材库写入文件。
...
- “设置”：左侧栏直接管理通用、外观、行为、桌面互动、宠物群、对话和只读宠物素材库；宠物大小与始终置顶也只在这里设置，仅真实连续动画显示视频混合设置。
- “宠物素材库”：切换内置宠物并查看档案；运行时不能导入、导出、替换或删除文件。
